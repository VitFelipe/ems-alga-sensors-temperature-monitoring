...
public class SensorMonitoringController {
    ...
    public ResponseEntity<SensorMonitoringOutput> disable(@PathVariable TSID sensorId) {
        SensorMonitoring sensorMonitoring = findByOrDefault(sensorId);
        if (!sensorMonitoring.getEnabled()) {
            // Sensor já está desabilitado, retorna 409 Conflict
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(toModel(sensorMonitoring));
        }
        sensorMonitoring.disable();
        sensorMonitoringRepository.save(sensorMonitoring);
        return ResponseEntity.ok(toModel(sensorMonitoring));
    }
